...
namespace Business.Validators
{
    public class CreateCourseReviewInputValidator : AbstractValidator<CreateCourseReviewInput>
    {
        ...
        public CreateCourseReviewInputValidator(IProfessorRepository professorRepository, ICourseRepository courseRepository, IReviewerInfoProvider reviewerInfoProvider, ICourseReviewRepository courseReviewRepository)
        {
        ...
        private async Task<bool> ReviewerHasNotAlreadyReviewedAsync(CreateCourseReviewInput input, CancellationToken cancellationToken)
        {
            var reviewer = await _reviewerInfoProvider.GetReviewer();
            var existingReview = await _courseReviewRepository.GetSingleOrDefaultAsync(r => r.CourseId == input.CourseId && r.ReviewerId == reviewer.Id);
